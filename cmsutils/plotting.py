...
if __name__ == "__main__":
    ...
    res["data"] = json.load(open(dd + "/data_2017.json"))
    for mc_samp in mc_samples:
        res[mc_samp] = json.load(open(dd + "/{0}.json".format(mc_samp)))
...
#    for analysis in ["baseline", "muon_tight", "subleading_muon_pt15", "no_jet_mu_clean", "jet_pt_40"]:
    for analysis in ["baseline"]:
        for var in [k for k in res["dy"][analysis].keys() if k.startswith("hist_")]:
            ...
            for weight in ["nominal", "leptonsf"]:
                ...
                plt.savefig(dd + "/{0}_{1}_{2}.pdf".format(analysis, var, weight), bbox_inches="tight")
