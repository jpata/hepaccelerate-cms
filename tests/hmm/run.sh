...
PYTHONPATH=coffea:hepaccelerate:. NUMBA_THREADING_LAYER=tbb NUMBA_ENABLE_AVX=1 NUMBA_NUM_THREADS=$NTHREADS OMP_NUM_THREADS=$NTHREADS HEPACCELERATE_CUDA=0 time singularity exec --nv -B /storage /bigdata/shared/Software/singularity/gpuservers/singularity/images/cupy.simg python3 tests/hmm/analysis_hmumu.py --action cache --maxfiles $MAXFILES --chunksize 1 --nthreads $NTHREADS --cache-location $CACHE_PATH --datapath /storage/user/jpata/

##Step 2: call physics code
PYTHONPATH=coffea:hepaccelerate:. NUMBA_THREADING_LAYER=tbb NUMBA_ENABLE_AVX=1 NUMBA_NUM_THREADS=$NTHREADS OMP_NUM_THREADS=$NTHREADS time singularity exec --nv -B /nvme1/ -B /storage /bigdata/shared/Software/singularity/gpuservers/singularity/images/cupy.simg python3 tests/hmm/analysis_hmumu.py --action analyze --maxfiles $MAXFILES --chunksize 10 --cache-location $CACHE_PATH --datapath /storage/user/jpata/ --pinned --async-data --nthreads $NTHREADS

##Step3: make plots
PYTHONPATH=coffea:hepaccelerate:. singularity exec --nv -B /storage /bigdata/shared/Software/singularity/gpuservers/singularity/images/cupy.simg python3 cmsutils/plotting.py
